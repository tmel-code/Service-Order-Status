...
st.markdown("This app counts **Unique Orders** and shows a Summary on the left.")
...
def load_data(file, header_idx):
    ...
    for engine, sep in methods:
        try:
            file.seek(0)
            if sep: 
                return pd.read_csv(file, header=header_idx, sep=sep, engine=engine)
            else: 
                return pd.read_excel(file, sheet_name=0, header=header_idx, engine=engine)
        except: 
            continue
    return None
